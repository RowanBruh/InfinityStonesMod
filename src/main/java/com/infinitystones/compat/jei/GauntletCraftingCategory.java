...
/**
 * JEI Recipe Category for Gauntlet crafting
 */
public class GauntletCraftingCategory implements IRecipeCategory<InfinityStoneRecipe> {
    public static final RecipeType<InfinityStoneRecipe> RECIPE_TYPE = 
            RecipeType.create(InfinityStonesMod.MOD_ID, "gauntlet_crafting", InfinityStoneRecipe.class);
    
    private static final ResourceLocation BACKGROUND_TEXTURE = 
            new ResourceLocation(InfinityStonesMod.MOD_ID, "textures/gui/jei/gauntlet_crafting.png");
    
    private final IDrawable background;
    private final IDrawable icon;
    
    public GauntletCraftingCategory(IGuiHelper guiHelper) {
        this.background = guiHelper.createDrawable(BACKGROUND_TEXTURE, 0, 0, 176, 85);
        this.icon = guiHelper.createDrawableIngredient(
                RecipeIngredientRole.RENDER_ONLY, 
                new ItemStack(ModItems.INFINITY_GAUNTLET.get())
        );
    }
    ...
    public Component getTitle() {
        return Component.translatable("infinitystones.jei.category.gauntlet_crafting");
    }
