...
/**
 * JEI Recipe Category for Skiddzie Items crafting
 */
public class SkiddzieItemCategory implements IRecipeCategory<InfinityStoneRecipe> {
    public static final RecipeType<InfinityStoneRecipe> RECIPE_TYPE = 
            RecipeType.create(InfinityStonesMod.MOD_ID, "skiddzie_items", InfinityStoneRecipe.class);
    
    private static final ResourceLocation BACKGROUND_TEXTURE = 
            new ResourceLocation(InfinityStonesMod.MOD_ID, "textures/gui/jei/skiddzie_items.png");
    
    private final IDrawable background;
    private final IDrawable icon;
    
    public SkiddzieItemCategory(IGuiHelper guiHelper) {
        this.background = guiHelper.createDrawable(BACKGROUND_TEXTURE, 0, 0, 176, 85);
        this.icon = guiHelper.createDrawableIngredient(
                RecipeIngredientRole.RENDER_ONLY, 
                new ItemStack(SkiddzieItems.ROWAN_LUCKY_BLOCK.get())
        );
    }
    ...
    public Component getTitle() {
        return Component.translatable("infinitystones.jei.category.skiddzie_items");
    }
